...
def _score_relevance(competition: dict) -> int:
    ...
    # Count weighted keyword hits
    high_hits = sum(1 for kw in HIGH_PRIORITY_KEYWORDS if kw in text)
    med_hits = sum(1 for kw in MEDIUM_PRIORITY_KEYWORDS if kw in text)
...
def _score_portfolio(competition: dict) -> int:
    ...
    # Prestigious organizer
    if any(org in text for org in PRESTIGIOUS_ORGS):
        score += 8

    # Research / benchmark potential
    research_hits = sum(1 for kw in RESEARCH_KEYWORDS if kw in text)
    ...
    if any(s in text for s in real_world_signals):
        score += 4
...
def should_notify(competition: dict) -> bool:
    """Return True if total_score meets the notification threshold."""
    return competition.get("total_score", 0) >= MIN_NOTIFY_SCORE
